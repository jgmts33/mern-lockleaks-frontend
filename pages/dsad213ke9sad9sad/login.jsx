...
export default function Login() {
    ...
    const handleLogin = useCallback(async () => {
        ...
        if (res.status == "success") {
            if ( res.data.roles.find(p => p == 'moderator') ) {
                setModalValue({
                    status: "success",
                    content: "Congratulations!, Go to dashboard with click below button",
                    userInfo: { ...res.data }
                })
            }
            else setModalValue({
                status: "failed",
                content: "You are not Moderator. Please go to user login page."
            });
            onOpen();
        } else {
            setModalValue({
